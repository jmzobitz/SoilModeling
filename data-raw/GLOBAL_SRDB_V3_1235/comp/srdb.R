...
# mapping functions slightly modified by ORNL DAAC -- July 2014
...
loaded <- vector()
for( lib in c( "ggplot2", "reshape2", "maps", "mapdata" ) ) {
	printlog( "Loading", lib )
	loaded[ lib ] <- require( lib, character.only=T )
	if( !loaded[ lib ] )
		warning( "this package is not installed!" )
}

printlog( "Reading data file..." )
srdb <- read.csv( "srdb-data-V3.csv" )
...
printlog( "All done with error checking." )
...
stopifnot( loaded[ "ggplot2" ] )
theme_set( theme_bw() )

# -----------------------------------------------------------------------------
# Two simple maps

if( loaded[ "maps" ] & loaded[ "mapdata" ] ) {
	printlog( "Making maps..." )
	world <- map_data("world")
	srdb$long <- srdb$Longitude
	srdb$lat <- srdb$Latitude
	p1 <- ggplot( srdb, aes( x=long, y=lat ) ) + 
	  geom_polygon( data=world, aes( group=group ), colour="grey70", fill= "grey70" ) +
	  geom_point(aes(fill=Biome), colour="black",pch=21, size=2) +
	...
	p2 <- p1 + coord_fixed( xlim=c( -160, -50 ), ylim=c( 15, 75 ) )
	print( p2 )
	ggsave( "map2.pdf" )
}
